...
#if SPSC == 1
        rcc = Push_SpscRingBuf(r, &o, chose_test_type_producer, arr, buf, &o);
#elif COMMIT  == 1
#if TRY == 1
        rcc = Try_push_MpscRingBuf(r, &o, chose_test_type_producer, arr, buf, &o);
#else
        rcc = Push_MpscRingBuf(r, &o, chose_test_type_producer, arr, buf, &o);
#endif
#elif SLOT == 1
        rcc = Try_push_MpmcRingBuf(r, &o, chose_test_type_producer, arr, buf, &o);
#endif
        if (rcc >= 0) {
...
int main()
{
    ...
    atomic_int coreN;
    if (PRO_START_CORE < 0) {
        PRO_START_CORE = CON_START_CORE + CON_THD_NUM * CORE_JUMP;
    }
